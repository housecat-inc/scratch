...
func TestTopNav(t *testing.T) {
	tests := []struct {
		...
		{name: "root uses shared navigation", path: "/", active: ""},
	...
	for _, tc := range tests {
		t.Run(tc.name, func(t *testing.T) {
			...
			body := rec.Body.String()
			a.Contains(body, `>Pages</span>`)
			a.Contains(body, `>Workflows</span>`)
			a.Contains(body, `>Setup</span>`)
			a.Contains(body, `href="/pages"`)

			if tc.active == "setup" {
				a.Contains(body, `href="/setup" class="gm-label active gm-label-child"`)
			} else {
				a.Contains(body, "Sessions")
			}
		})
	}
}
