...
func (f *Flows) contactIntake(ctx dbos.DBOSContext, in ContactInput) (string, error) {
	...
	draft, err := dbos.RunAsStep(ctx, func(stepCtx context.Context) (Contact, error) {
		stepCtx, cancel := context.WithTimeout(stepCtx, time.Minute)
		defer cancel()
	...
	case elicit.ActionAccept:
		...
		task, err := dbos.RunAsStep(ctx, func(context.Context) (db.Task, error) {
			return f.deps.Tasks.AddWorkflowTask(turnWorkflowID(in.MessageID)+"/task", followUpTitle(contact))
		}, dbos.WithStepName("add_task"), dbos.WithStepMaxRetries(2))
		if err != nil {
...
func (f *Flows) delta(ctx dbos.DBOSContext, in ContactInput, text string) error {
	_, err := dbos.RunAsStep(ctx, func(context.Context) (string, error) {
		...
		if err := f.deps.Store.AddWorkflowMessageEvent(fmt.Sprintf("%s/delta/%x", turnWorkflowID(in.MessageID), sha256.Sum256([]byte(text))), in.MessageID, ev.Type, ev.Data, text); err != nil {
			return "", err
		}
		f.deps.Publish(in.ThreadID)
		return "", nil
	}, dbos.WithStepName("delta"), dbos.WithStepMaxRetries(2))
...
func (f *Flows) finish(ctx dbos.DBOSContext, in ContactInput) error {
	_, err := dbos.RunAsStep(ctx, func(context.Context) (string, error) {
		...
		f.deps.Publish(in.ThreadID)
		return "", nil
	}, dbos.WithStepName("finish"), dbos.WithStepMaxRetries(2))
...
func (f *Flows) toolCall(ctx dbos.DBOSContext, in ContactInput, name string, input map[string]string) error {
	_, err := dbos.RunAsStep(ctx, func(context.Context) (string, error) {
		...
		if err := f.deps.Store.AddWorkflowMessageEvent(turnWorkflowID(in.MessageID)+"/tool/"+name, in.MessageID, chat.EventToolCall, string(data), ""); err != nil {
			return "", err
		}
		f.deps.Publish(in.ThreadID)
		return "", nil
	}, dbos.WithStepName("tool_call/"+name), dbos.WithStepMaxRetries(2))
...
func (p prompter) Prompt(pr elicit.Prompt) error {
	...
	if err := p.deps.Store.AddWorkflowMessageEvent(pr.ElicitationID, p.in.MessageID, chat.EventElicitation, string(data), ""); err != nil {
